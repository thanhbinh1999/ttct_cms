        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <a href="" class="kt-subheader__breadcrumbs-link">Bình luận theo bài viết: <strong>{{ $article['title'] }}</strong></a>
                </div>
            </div>
            <div class="kt-subheader__toolbar" style="display: none" data-status="published">
                <div class="kt-subheader__wrapper">
                    @can('comment-update-content-and-author')<button class="btn btn-success" id="bulk-update-content-and-author" data-url="{{ route('comments.update_comment_and_author') }}"><i class="la la-check"></i> Bulk Cập nhật nôi dung & tác giả</button>@endcan
                    ...
                    @can('comment-delete')<button type="submit" class="btn btn-warning btn-md" id="bulk-delete" data-url="{{ route('comments.delete') }}"><i class="la la-close"></i>Bulk Xóa</button>@endcan
                </div>
            </div>
            <div class="kt-subheader__toolbar" style="display: none" data-status="verified">
                <div class="kt-subheader__wrapper">
            ...
            <div class="kt-subheader__toolbar" style="display: none" data-status="pending">
                <div class="kt-subheader__wrapper">
                    @can('comment-verify')<button class="btn btn-success" id="bulk-verify" data-url="{{ route('comments.verify') }}"><i class="la la-check"></i> Bulk Duyệt</button>@endcan
                    @can('comment-delete')<button class="btn btn-warning btn-md" id="bulk-delete" data-url="{{ route('comments.delete') }}"><i class="la la-close"></i>Bulk Xóa</button>@endcan
                </div>
            </div>
            <div class="kt-subheader__toolbar" style="display: none" data-status="deleted">
                <div class="kt-subheader__wrapper">
                    @can('comment-restore')<button class="btn btn-success" id="bulk-restore" data-url="{{ route('comments.restore') }}"><i class="la la-undo"></i> Bulk khôi phục</button>@endcan
                </div>
            </div>
        </div>
            ...
            <div class="kt-portlet kt-portlet--mobile">
                <div class="kt-portlet__head kt-portlet__head--lg">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title text-uppercase">Bình luận theo bài viết: {{ $article['title'] }}</h3>
                    </div>
                </div>
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-md-4 kt-margin-b-20-tablet-and-mobile">
                                            <div class="form-group row">
                                                <label for="search-to" class="col-form-label col-3">Trạng thái</label>
                                                <div class="col-9">
                                                    <select class="form-control" id="search-status">
                                                        <option value="published" selected>Đã xuất bản</option>
                                                        <option value="verified">Chờ xuất bản</option>
                ...
                <div class="kt-portlet__body kt-portlet__body--fit">
                    <div class="kt-datatable" id="comment-by-article-table" data-id="{{ $id }}" data-url="{{ route('comments.comment_by_article') }}"></div>
                </div>
            </div>
        </div>
    </div>
...
@stop
@push('page-js')
    <script src="{{ asset('assets/js/pages/comment-by-article.js') }}" type="text/javascript"></script>
@endpush
