...
class CommentReported {
        ...
        this.datatable = this.$commentReportedTable.KTDatatable({
            ...
            columns: [
                    ...
                    template: row => {
                        let content = row.sticker ? `
                                        <div class="row">
                                            ...
                                            <div class="col-8" style="color: black; font-size: 15px">
                                                <div data-content-id="${row.id}"  style="height: 100%">
                                                ${row.content}
                                                </div>
                                            </div>
                                        </div>` : `
                        ...
                        return `
                            <div data-container-id="${row.id}">
                                ...
                                <div class="row mt-3">
                                    <div class="col-12">
                                        <p style="color: black"><strong style="color: red" data-author-id="${row.id}">${row.author_name}</strong> ${row.author_email}</p>
                                    </div>
                                    <div class="col-12">
                ...
                },
                {
                    ...
                    sortable: false,
                    textAlign: 'center',
                    template: row => (row.reports && row.reports.length > 0) ? (row.reports.reduce((c, n) => c + `<p><span ${parseInt(n.status) === 2 ? ' style="font-weight:bold"' : ''}>${n.author_email}</span>: ${n.content}</p>`, '<div style="color: black">') + '</div>') : 'N/A'
                },
                {
                    field: 'created_at',
                    title: 'NGÀY TẠO',
                ...
                },
                {
                    ...
                    template: row => {
                        if (this.acl) {
                            let permissions = checkPermissions(['comment-view-log', 'comment-report-delete', 'comment-report-ignore'], this.acl.permissions);
                            let del = permissions['comment-report-delete'] ? `<a class="dropdown-item font-weight-bold delete" href="#" data-url-delete="${row.report_delete}" data-id="${row.id}"><i class="la la-close"></i> Xóa</a>` : '';
                            let ignore = permissions['comment-report-ignore'] ? `<a class="dropdown-item font-weight-bold ignore" href="#" data-url-ignore="${row.report_ignore}" data-id="${row.id}"><i class="la la-check"></i> Bỏ qua</a>` : '';
    ...
    initIgnore() {
        $(document).on('click', '.ignore', event => {
            event.preventDefault();
            let url = $(event.currentTarget).data('url-ignore');
            let id = $(event.currentTarget).data('id');

            swal.fire({
                title: 'Xác nhận bỏ qua',
                html: `<p style='font-size: 15px;color: black'>Bạn muốn bỏ qua các báo cáo về bình luận này?</p>`,
                type: 'warning',
        ...
        this.$bulkIgnore.click(event => {
            ...
            if (this.selectedData && this.selectedData.length > 0) {
                ...
                swal.fire({
                    title: 'Xác nhận bỏ qua',
                    html: `<p style='font-size: 15px;color: black'>Bạn muốn bỏ quá các báo cáo về ${data.length} bình luận này?</p>`,
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonText: 'OK',
                    cancelButtonText: 'Hủy',
