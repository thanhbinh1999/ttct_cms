...
/***/ "./platform/ttct/resources/assets/js/pages/article-index-take-down.js":
/*!****************************************************************************!*\
  !*** ./platform/ttct/resources/assets/js/pages/article-index-take-down.js ***!
  \****************************************************************************/
/*! no exports provided */
...
function () {
  function ArticleIndexPublished() {
    ...
    this.currnenSendback = null; //transfer
  ...
  _createClass(ArticleIndexPublished, [{
    ...
    value: function initTransfer() {
      ...
      this.$modalTransferArticle.on('show.bs.modal', function (event) {
        _this.currentTrasnferArticle = $(event.relatedTarget).data('id');
      ...
      this.$transferType.change(function (event) {
        if ($(event.target).val() === 'role') {
          _this.transferType = 'role';

          _this.$userSelect.parents('.form-group').hide();

          _this.initRoleSelect();
        } else {
          _this.transferType = 'user';
      ...
      this.$formTransferCategory.submit(function (event) {
        ...
        if (_this.transferType === 'role') {
          if (_this.$roleSelect.select2('data').length === 0) {
            toastr.error('Chưa chọn vai trò');
            return false;
          }
        } else {
          if (_this.$userSelect.select2('data').length === 0) {
        ...
        var urlUpdateArticleEditor = _this.$articleTable.data('update-article-editor') + _this.currentTrasnferArticle;

        var fd = new FormData(event.target);
        fd.append('type', _this.transferType);
        fd.append('article_id', _this.currentTrasnferArticle);
        fd.append('from_user', _this.$userInfo.data('id'));
        fd.append('from_user_full_name', _this.$userInfo.data('full-name'));
        fd.append('from_user_username', _this.$userInfo.data('username'));
        fd.append('note', _this.$note.val());
        fd.append('sb_or_fw', 1);
        var listUser = [{
          id: _this.$userInfo.data('id'),
          full_name: _this.$userInfo.data('full-name'),
          username: _this.$userInfo.data('username')
        }];

        if (_this.transferType === 'role') {
          fd.append('to_user', _this.$roleSelect.select2('data')[0].id);
          fd.append('to_user_full_name', _this.$roleSelect.select2('data')[0].title);
          fd.append('to_user_username', _this.$roleSelect.select2('data')[0].title);
        } else {
          fd.append('to_user', _this.$userSelect.select2('data')[0].id);
          fd.append('to_user_full_name', _this.$userSelect.select2('data')[0].full_name);
          fd.append('to_user_username', _this.$userSelect.select2('data')[0].username);
          listUser.push({
            id: _this.$userSelect.select2('data')[0].id,
            full_name: _this.$userSelect.select2('data')[0].full_name,
            username: _this.$userSelect.select2('data')[0].username
          });
        }

        fd.append('list_user', JSON.stringify(listUser));
        var arr = [axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(url, fd), axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(urlUpdateArticleEditor, {})];
        Promise.all(arr).then(function (value) {
          if (value[0].data.message) {
            toastr.success(value[0].data.message);

            _this.$modalTransferArticle.modal('hide');
    ...
    value: function initDatatable() {
      ...
      this.datatable = this.$articleTable.KTDatatable({
        ...
        columns: [{
          ...
          textAlign: 'left',
          template: function template(row) {
            return row.categories && row.categories[0] !== undefined ? "<strong>".concat(row.categories[0].name, " </strong>") : 'N/A';
          }
        }, {
          field: 'author',
          title: 'TÁC GIẢ',
          ...
          textAlign: 'left',
          template: function template(row) {
            return row.author ? "<strong>".concat(row.author, "</strong>") : 'N/A';
          }
        }, {
          field: 'creator',
          title: 'NGƯỜI TẠO',
          ...
          textAlign: 'left',
          template: function template(row) {
            return row.creator ? "<strong>".concat(row.creator.username, "</strong>") : 'N/A';
          }
        }, {
          field: 'published_at',
          title: 'XUẤT BẢN',
          ...
          textAlign: 'center',
          template: function template(row) {
            return row.published_at ? row.published_at : 'N/A';
          }
        }, {
          field: 'action',
          title: '!!!',
          ...
          template: function template(row) {
            ...
            if (_this3.acl) {
              var permissions = Object(_helper__WEBPACK_IMPORTED_MODULE_1__["checkPermissions"])(['article-update', 'article-publish', 'article-restore', 'article-send-back'], _this3.acl.permissions);
              ...
              if (_this3.$userInfo.data('role-name') === 'tkts') {
                edit = "<a class=\"dropdown-item edit font-weight-bold\" href=\"".concat(editUrl, "\"><i class=\"la la-edit\"></i> C\u1EADp nh\u1EADt</a>");

                if (parseInt(_this3.$userInfo.data('id')) === parseInt(row.creator.id)) {
                  re = permissions['article-restore'] ? "<a class=\"dropdown-item restore font-weight-bold\" href=\"#\" data-id=\"".concat(row.id, "\"  data-restore=\"").concat(row.url_restore, "\"><i class=\"la la-undo\"></i> Kh\xF4i ph\u1EE5c</a>") : '';
                } else if (!row.editor.id) {
                  sendBack = "<a class=\"dropdown-item send-back font-weight-bold\" data-data='".concat(JSON.stringify(row), "' href=\"#\" data-toggle=\"modal\" data-target=\"#modal-send-back\"><i class=\"la la-mail-reply\"></i> Tr\u1EA3 l\u1EA1i</a>");
                }
              } else {
                if (row.editor && parseInt(row.editor.id) === parseInt(row.creator.id)) {
                  edit = "<a class=\"dropdown-item edit font-weight-bold\" href=\"".concat(editUrl, "\"><i class=\"la la-edit\"></i> C\u1EADp nh\u1EADt</a>");
                  trans = "<a class=\"dropdown-item font-weight-bold\" href=\"#\" data-toggle=\"modal\" data-target=\"#modal-transfer-article\" data-id=\"".concat(row.id, "\"><i class=\"la la-edit\"></i> Chuy\u1EC3n b\xE0i vi\u1EBFt</a>");
                }
              }
    ...
    value: function initRestore() {
      ...
      $(document).on('click', '.restore', function (event) {
        event.preventDefault();
        var url = $(event.currentTarget).data('restore');
        swal.fire({
    ...
    value: function initSendBackSelectUser() {
      var html = "<option value=\"".concat(this.currnenSendback.creator.id, "\" data-username=\"").concat(this.currnenSendback.creator.username, "\" data-full_name=\"").concat(this.currnenSendback.creator.full_name, "\">").concat(this.currnenSendback.creator.username, "</option>");
    ...
    value: function initSendBack() {
      ...
      this.$formSendBack.submit(function (event) {
        ...
        var fd = new FormData(event.target);
        fd.append('type', 'user');
        fd.append('article_id', _this5.currnenSendback.id);
        ...
        var listUser = [{
          ...
          id: _this5.$sendBackUserSelect.select2('data')[0].id,
          full_name: $(_this5.$sendBackUserSelect.select2('data')[0].element).data('full_name'),
          username: $(_this5.$sendBackUserSelect.select2('data')[0].element).data('username')
        }];
        fd.append('list_user', JSON.stringify(listUser));
        axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(url, fd).then(function (_ref) {
          var data = _ref.data;
          ...
          data: function data(params) {
            return {
              ...
              page: params.page,
                ...
                if (_this6.$userInfo.data('role-name') === 'btv') return "[1,".concat(_this6.$userInfo.data('role-id'), ",2]");else return "[1,".concat(_this6.$userInfo.data('role-id'), "]");
              }
            };
          },
          ...
          data: function data(params) {
            return {
              ...
              roles: function roles() {
                if (_this7.$userInfo.data('role-name') === 'btv') return "[1,".concat(_this7.$userInfo.data('role-id'), ",2]");else return "[1,".concat(_this7.$userInfo.data('role-id'), "]");
              }
            };
          },
          processResults: function processResults(data) {
...
/***/ 3:
/*!**********************************************************************************!*\
  !*** multi ./platform/ttct/resources/assets/js/pages/article-index-take-down.js ***!
  \**********************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/ttct_cms/platform/ttct/resources/assets/js/pages/article-index-take-down.js */"./platform/ttct/resources/assets/js/pages/article-index-take-down.js");
