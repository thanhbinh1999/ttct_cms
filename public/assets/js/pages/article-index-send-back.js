...
/***/ "./platform/ttct/resources/assets/js/pages/article-index-send-back.js":
/*!****************************************************************************!*\
  !*** ./platform/ttct/resources/assets/js/pages/article-index-send-back.js ***!
  \****************************************************************************/
/*! no exports provided */
...
var ArticleIndexSendBack =
/*#__PURE__*/
function () {
  function ArticleIndexSendBack() {
    _classCallCheck(this, ArticleIndexSendBack);
  ...
  _createClass(ArticleIndexSendBack, [{
    key: "init",
    value: function init() {
      this.initGetAcl();
      ...
      this.datatable = this.$articleTableSendBack.KTDatatable({
            ...
            read: {
              ...
              params: {
                query: (_query = {}, _defineProperty(_query, 'search-key', function searchKey() {
                  ...
                  return _this2.$userInfo.data('id');
                }), _defineProperty(_query, "sb_or_fw", 2), _query)
              },
        ...
        columns: [{
          ...
          template: function template(row) {
            return "<strong>".concat(row.article.title, "</strong>");
          }
        }, {
          field: 'note',
          title: 'GHI CHÚ',
          ...
          textAlign: 'left',
          template: function template(row) {
            return "<strong>".concat(row.to_user.username ? row.to_user.username : 'N/A', "</strong>");
          }
        }, {
          field: 'status',
          title: 'TRẠNG THÁI',
          ...
          template: function template(row) {
            if (_this2.acl) {
              var permissions = Object(_helper__WEBPACK_IMPORTED_MODULE_1__["checkPermissions"])(['article-delete', 'article-publish', 'article-update', 'article-transfer', 'article-update-own', 'article-send-back'], _this2.acl.permissions);
              var html = '';

              if (parseInt(row.type) === 2 || parseInt(row.type) === 1) {
                if (parseInt(row.status) === 2 && row.list_user.length === 2 || parseInt(row.status) === 2 && _this2.$userInfo.data('role-name') === 'btv' || parseInt(row.status) === 2 && _this2.$userInfo.data('role-name') === 'pv') {
                  html = "<a class=\"dropdown-item mask-received font-weight-bold\" href=\"#\" data-data='".concat(JSON.stringify(row), "' data-mask-processing=\"").concat(row.url_mask_received, "\" data-status=\"").concat(row.mask_received_status, "\"><i class=\"la la-check\"></i> \u0110\xE1nh d\u1EA5u \u0111\xE3 nh\u1EADn</a>");
                } else if (parseInt(row.status) === 2 && row.list_user.length > 2) {
                  var pos = 0;

                  for (var i = pos; i < row.list_user.length - 1; i++) {
                    if (parseInt(row.list_user[i].id) === parseInt(_this2.$userInfo.data('id'))) {
                      pos = i;
                      break;
                    }
                  }

                  if (pos > 0 && pos < row.list_user.length - 1) html = permissions['article-send-back'] ? "<a class=\"dropdown-item send-back font-weight-bold\" data-data='".concat(JSON.stringify(row), "' href=\"#\" data-toggle=\"modal\" data-target=\"#modal-send-back\"><i class=\"la la-mail-reply\"></i> Tr\u1EA3 l\u1EA1i & \u0111\xE1nh d\u1EA5u \u0111\xE3 x\u1EED l\xFD</a>") : '';
                } else if (parseInt(row.status) === 2 && parseInt(row.article.status) === 6) {
                  html = "<a class=\"dropdown-item mask-received font-weight-bold\" href=\"#\" data-data='".concat(JSON.stringify(row), "' data-mask-processing=\"").concat(row.url_mask_received, "\" data-status=\"").concat(row.mask_received_status, "\"><i class=\"la la-check\"></i> \u0110\xE1nh d\u1EA5u \u0111\xE3 nh\u1EADn</a>");
                }
              } else {
                html = "<a class=\"dropdown-item mask-received font-weight-bold\" href=\"#\" data-mask-processing=\"".concat(row.url_mask_received, "\" data-status=\"").concat(row.mask_received_status, "\"><i class=\"la la-check\"></i> \u0110\xE1nh d\u1EA5u \u0111\xE3 nh\u1EADn</a>");
              }
    ...
    key: "maskReceivedInit",
    value: function maskReceivedInit() {
      var _this3 = this;

      $(document).on('click', '.mask-received', function (event) {
        ...
        if (data && parseInt(data.article.status) === 6) {
          var urlUpdateEditor = _this3.$articleTableSendBack.data('update-article-editor') + data.article.id;
          var arr = [axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(url, {
            status: status
          }), axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(urlUpdateEditor, {
            user_id: _this3.$userInfo.data('id'),
            user_full_name: _this3.$userInfo.data('full-name'),
    ...
    value: function initSendBack() {
      ...
      this.$formSendBack.submit(function (event) {
        event.preventDefault();
        var urlMaskProcessed = _this4.currnenSendback.url_mask_processed;
        var status = _this4.currnenSendback.mask_processed_status;
        var url = $(event.target).attr('action');
        var fd = new FormData(event.target);
        fd.append('type', 'user');
        fd.append('article_id', _this4.currnenSendback.article.id);
        fd.append('from_user', _this4.$userInfo.data('id'));
        fd.append('from_user_full_name', _this4.$userInfo.data('full-name'));
        fd.append('from_user_username', _this4.$userInfo.data('username'));
        fd.append('note', _this4.$sendBackNote.val());
        fd.append('sb_or_fw', 2);
        fd.append('to_user', _this4.$sendBackUserSelect.select2('data')[0].id);
        fd.append('to_user_full_name', $(_this4.$sendBackUserSelect.select2('data')[0].element).data('full_name'));
        fd.append('to_user_username', $(_this4.$sendBackUserSelect.select2('data')[0].element).data('username'));
        var listUser = [];
        var length = _this4.currnenSendback.list_user.length;

        if (parseInt(_this4.currnenSendback.list_user[length - 1].id) === parseInt(_this4.$userInfo.data('id'))) {
          listUser = _this4.currnenSendback.list_user;
        } else {
          listUser = [{
            id: _this4.$userInfo.data('id'),
            full_name: _this4.$userInfo.data('full-name'),
            username: _this4.$userInfo.data('username')
          }, {
            id: _this4.$sendBackUserSelect.select2('data')[0].id,
            full_name: $(_this4.$sendBackUserSelect.select2('data')[0].element).data('full_name'),
            username: $(_this4.$sendBackUserSelect.select2('data')[0].element).data('username')
          }];
        }
        ...
        Promise.all(arr).then(function (value) {
          ...
          if (value[1].data.message) {
            ...
            _this4.$modalSendBack.modal('hide');
      ...
      this.$modalSendBack.on('show.bs.modal', function (event) {
        _this4.currnenSendback = $(event.relatedTarget).data('data');

        _this4.initSendBackSelectUser();
      }).on('hide.bs.modal', function () {
        if (_this4.$sendBackUserSelect.hasClass("select2-hidden-accessible")) {
          _this4.$sendBackUserSelect.select2('destroy');
        }

        _this4.$sendBackUserSelect.html('');

        _this4.$sendBackNote.val('');
      });
    }
  }, {
    ...
    value: function initSendBackSelectUser() {
      ...
      for (var i = pos; i < this.currnenSendback.list_user.length - 1; i++) {
      ...
      for (var _i = pos - 1; _i >= 0; _i--) {
        listUser.push(this.currnenSendback.list_user[_i]);
      }
...
/***/ 7:
/*!**********************************************************************************!*\
  !*** multi ./platform/ttct/resources/assets/js/pages/article-index-send-back.js ***!
  \**********************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/ttct_cms/platform/ttct/resources/assets/js/pages/article-index-send-back.js */"./platform/ttct/resources/assets/js/pages/article-index-send-back.js");
