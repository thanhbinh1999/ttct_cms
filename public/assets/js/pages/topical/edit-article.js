...
/***/ "./platform/ttct/resources/assets/js/pages/topical/edit-article.js":
/*!*************************************************************************!*\
  !*** ./platform/ttct/resources/assets/js/pages/topical/edit-article.js ***!
  \*************************************************************************/
/*! no exports provided */
...
function () {
  ...
  _createClass(EditArticle, [{
    ...
    value: function init() {
      ...
      Object(_helper__WEBPACK_IMPORTED_MODULE_0__["showBlock"])();
    ...
    value: function initSelect2() {
      ...
      this.$categories.on('select2:select', function () {
        var formGroup = _this3.$categories.parent();

        formGroup.find('.invalid-feedback').remove();

        _this3.$categories.removeClass('is-invalid');
      });
      this.$tags.on('select2:select', function () {
        var formGroup = _this3.$tags.parent();

        formGroup.find('.invalid-feedback').remove();

        _this3.$tags.removeClass('is-invalid');
      });
    }
  }, {
    key: "setFileMode",
    ...
    value: function initGetData() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_1___default.a.get($('#kt_content').data('edit')).then(function (_ref) {
        var data = _ref.data;
        _this4.data = data;
        _this4.thumb = data.thumbnail;

        _this4.$title.val(data.title);

        document.title = data.title + " " + document.title;

        _this4.$excerpt.val(data.excerpt);

        _this4.$author.val(data.author);

        data.metas.forEach(function (meta) {
          switch (meta.key) {
            case 'google-description':
              {
                _this4.$updateGoogleDescription.val(meta.value);
            ...
            case 'google-key':
              {
                _this4.$uploadGoogleKey.val(meta.value);
            ...
            case 'fb-description':
              {
                _this4.$uploadFbDescription.val(meta.value);
            ...
            case 'fb-title':
              {
                _this4.$uploadFbTitle.val(meta.value);
        ...
        var dropify = _this4.dropify.data('dropify');

        dropify.settings.defaultFile = "".concat(data.thumbnail.base_url, "/").concat(data.thumbnail.absolute_url);
        dropify.destroy();
        dropify.init();
        $('.dropify-clear').hide();

        _this4.$thumbnail.change(function () {
          $('.dropify-clear').show();
          _this4.thumb = null;
        });
        ...
        var valueCa = categories.map(function (item) {
          ...
          _this4.$categories.append(option);

          return item.id;
        });

        _this4.$categories.val(valueCa).trigger('change');

        _this4.$categories.prop('disabled', true);

        var valueTag = data.tags.map(function (tag) {
          var option = new Option(tag.name, tag.id);

          _this4.$tags.append(option);

          return tag.id;
        });

        _this4.$tags.val(valueTag).trigger('change');

        _this4.initEditor(_this4.data.content);

        setTimeout(function () {
          Object(_helper__WEBPACK_IMPORTED_MODULE_0__["hideBlock"])();
        }, 500);
      }, function (err) {});
    }
  }, {
    ...
    value: function initSubmit() {
      ...
      this.$btnSubmit.click(function (event) {
        ...
        var categories = _this5.$categories.select2('data');

        var tags = _this5.$tags.select2('data');

        fd.append('title', _this5.$title.val());
        fd.append('author', _this5.$author.val());
        fd.append('excerpt', _this5.$excerpt.val());
        fd.append('google-description', _this5.$updateGoogleDescription.val());
        fd.append('google-key', _this5.$uploadGoogleKey.val());
        fd.append('fb-description', _this5.$uploadFbDescription.val());
        fd.append('fb-title', _this5.$uploadFbTitle.val());
        ...
        if (_this5.thumb) {
          fd.append('thumb', _this5.thumb.id);
        } else if (_this5.$thumbnail[0].files[0]) {
          fd.append('thumbnail', _this5.$thumbnail[0].files[0]);
        }

        axios__WEBPACK_IMPORTED_MODULE_1___default.a.post(url, fd).then(function (res) {
          if (res.data && res.data.message) {
            toastr.success(res.data.message); // setTimeout(() => {
...
$(function () {
  ...
  new _article_create_upload_resource__WEBPACK_IMPORTED_MODULE_2__["default"](editArticle);
});
...
/***/ 23:
/*!*******************************************************************************!*\
  !*** multi ./platform/ttct/resources/assets/js/pages/topical/edit-article.js ***!
  \*******************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/ttct_cms/platform/ttct/resources/assets/js/pages/topical/edit-article.js */"./platform/ttct/resources/assets/js/pages/topical/edit-article.js");
