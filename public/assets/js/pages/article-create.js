...
/***/ "./platform/ttct/resources/assets/js/pages/article-create.js":
/*!*******************************************************************!*\
  !*** ./platform/ttct/resources/assets/js/pages/article-create.js ***!
  \*******************************************************************/
/*! no exports provided */
...
function () {
  function ArticleCreate() {
    ...
    this.$setRelatedArticles=  $('.list-article') 
    ...
    this.$offDescription = $('#off_description'); //select image
    ...
    this.$modalUploadResourceBtnUpload = $('#modal-upload-resource-btn-upload'); //
    this.$relatedSelectArticle = $(".list-article");
  ...
  _createClass(ArticleCreate, [{
    ...
    key: "initSelectImage",
    value: function initSelectImage() {
      var _this = this;
      ...
      this.$selectCoverPc.click(function (event) {
        event.preventDefault();
        _this.fileMode = 'image';
        _this.selectResourceIsCurrentUseFor = 'cover-pc';
      });
      this.$selectCoverMobile.click(function (event) {
        event.preventDefault();
        _this.fileMode = 'image';
        _this.selectResourceIsCurrentUseFor = 'cover-mobile';
      });
    }
  }, {
    key: "initSelectResource",
    value: function initSelectResource() {
      var _this2 = this;

      this.$selectResources.click(function (event) {
        event.preventDefault();
        _this2.fileMode = 'image';
        _this2.selectResourceIsCurrentUseFor = 'resources';
      });
    }
  }, {
    key: "initPreview",
    value: function initPreview() {
      ...
      this.$articlePreviewBtn.click(function (event) {
        ...
        _this3.setLocalStorageData();

        if (_this3.window) {
          _this3.window.location.reload();
        } else {
          _this3.window = window.open(url, '_blank');

          _this3.window.onbeforeunload = function () {
            _this3.window = null;
    ...
    value: function initPreventReload() {
      ...
      this.$createGoogleKey.on('keyup change', function () {
        if (!window.onbeforeunload) window.onbeforeunload = function () {
          return '';
        };
      });
      this.$createGoogleDescription.on('keyup change', function () {
        if (!window.onbeforeunload) window.onbeforeunload = function () {
          return '';
        };
      });
      this.$createFbTitle.on('keyup change', function () {
        if (!window.onbeforeunload) window.onbeforeunload = function () {
          return '';
        };
      });
      this.$createFbDescription.on('keyup change', function () {
        if (!window.onbeforeunload) window.onbeforeunload = function () {
          return '';
        };
      });
    ...
    value: function initFroalaEditor() {
      ...
      FroalaEditor.RegisterCommand('modalBrowseVideo', {
        ...
        callback: function callback() {
          _this4.selectResourceIsCurrentUseFor = 'froala';
          _this4.fileMode = 'video';

          _this4.$modalSelectResource.modal('show');
        }
      });
      FroalaEditor.RegisterCommand('modalSelectBlockHtml', {
        ...
        refreshAfterCallback: false,
        callback: function callback() {
          _this4.editor.html.insert('<div id="position_mark"></div>');

          _this4.$modalSelectBlock.modal('show');
        }
      });
      ...
      FroalaEditor.RegisterCommand('videoCaption', {
        ...
        callback: function () {
            ...
            $('#videoCaption-2').click(function(){
              $('.fr-video').addClass('fr-active');
              $('.fr-inner').text('caption');
            });

            $('#videoCaption-3').click(function(){
              $('.fr-video').addClass('fr-active');
              $('.fr-inner').text('caption');
            });
      ...
      FroalaEditor.RegisterQuickInsertButton('quickInsertButtonImage', {
        ...
        callback: function callback() {
          ...
          setQuickInsertSetHtmlFunc(_this4.editor.html.insert.bind(_this4.editor));
          setSelectResourceIsCurrentUseFor('froala-quick-insert');
          var model = setCurrentEditFile(null);
          setFileMode('image');
          processFileType();
    ...
    value: function initSubmit() {
      ...
      this.$btnSubmit.click(function (event) {
        ...
        if (!_this5.thumb && !_this5.$thumbnail[0].files[0]) {
          toastr.info('Chưa nhập hình đại diện');
          Object(_helper__WEBPACK_IMPORTED_MODULE_1__["hideBlock"])();
          return false;
        }

        if (_this5.$categories.select2('data').length === 0) {
          toastr.info('Chưa nhập chuyên mục');
          Object(_helper__WEBPACK_IMPORTED_MODULE_1__["hideBlock"])();
          return false;
        }

        var fd = _this5.generateFormData();

        if (_this5.$categories.select2('data')[0].id === 'reserve-paper') {
          url = _this5.$categories.data('store');
        }
        ...
        axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(url, fd).then(function (res) {
          if (res.data && res.data.message) {
            ...
            setTimeout(function () {
              window.location.href = _this5.$categories.select2('data').length > 0 && _this5.$categories.select2('data')[0].id === 'reserve-paper' ? _this5.$categories.data('reload') : _this5.$btnSubmit.data('reload');
            }, 1000);
          } else {
    ...
    value: function generateFormData() {
      ...
      var themes = this.$createTheme.select2('data');
      var relatedArticle =  this.$relatedSelectArticle.select2('data');
      ...
      if (this.$createPriority.length > 0) fd.append('priority', this.$createPriority.val());else {
        fd.append('priority', 0);
      }
      var content = $('.fr-element.fr-view');

      if (content.children('.fr-view').length > 0) {
        var tmp = content.children('.fr-view');
        var i = 0;
    ...
    value: function initDropify() {
      ...
      this.$thumbnail.click(function (event) {
        ...
        _this6.$selectThumbnail.click();
      });
      this.$coverPC.click(function (event) {
        event.preventDefault();

        _this6.$selectCoverPc.click();
      });
      this.$coverMobile.click(function (event) {
        event.preventDefault();

        _this6.$selectCoverMobile.click();
      });
    }
  }, {
    key: "setImageDropify",
    value: function setImageDropify(img, type) {
      switch (type) {
        case 'thumbnail':
          {
            var thumb = this.dropifyThumbnail.data('dropify');
    ...
    value: function initSelect2() {
      ...
      this.$categories.select2({
        ...
        ajax: {
          ...
          processResults: function processResults(data) {
            var tmpData = _objectSpread({}, data);

            if (_this7.$categories.data('reserve-paper') !== undefined) tmpData.data.push({
              name: 'TTCT - Kỳ báo',
              id: 'reserve-paper'
            });
      ...
      this.$categories.on('select2:select', function () {
        var formGroup = _this7.$categories.parent();

        formGroup.find('.invalid-feedback').remove();

        _this7.$categories.removeClass('is-invalid');

        var data = _this7.$categories.select2('data');

        if (data.length > 0 && data[0].id === 'reserve-paper') {
          _this7.$createPublishedAt.val('').prop('disabled', true);
          _this7.$createTheme.val('').prop('disabled',true);
          _this7.$tags.val('').prop('disabled',true);
          _this7.$createPriority.prop('disabled', true);
          _this7.$setRelatedArticles.val("").prop('disabled',true);
          _this7.$excerpt.val("").prop('disabled',true);
        } else {
          _this7.$createPublishedAt.prop('disabled', false);
          _this7.$createTheme.val('').prop('disabled',false);
          _this7.$createPriority.prop('disabled', false);
          _this7.$tags.val('').prop('disabled',false);
          _this7.$setRelatedArticles.val("").prop('disabled',false);
          _this7.$excerpt.val("").prop('disabled',false);
        }
      ...
      this.$tags.on('select2:select', function () {
        var formGroup = _this7.$tags.parent();

        formGroup.find('.invalid-feedback').remove();

        _this7.$tags.removeClass('is-invalid');

        if (_this7.$createTheme.select2('data').length > 0) {
          if (!window.onbeforeunload) window.onbeforeunload = function () {
            return '';
          };
        } else {
      ...
      this.$createTheme.on('select2:select', function () {
        var formGroup = _this7.$createTheme.parent();

        formGroup.find('.invalid-feedback').remove();

        _this7.$createTheme.removeClass('is-invalid');
...
$(function () {
  var articleCreate = new ArticleCreate();
  new _article_create_upload_resource__WEBPACK_IMPORTED_MODULE_2__["default"](articleCreate);
  new _article_create_update_create_tag__WEBPACK_IMPORTED_MODULE_3__["default"](articleCreate.$tags);
  new _article_create_update_create_theme__WEBPACK_IMPORTED_MODULE_4__["default"](articleCreate.$createTheme);
});
...
/***/ 9:
/*!*************************************************************************!*\
  !*** multi ./platform/ttct/resources/assets/js/pages/article-create.js ***!
  \*************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/ttct_cms/platform/ttct/resources/assets/js/pages/article-create.js */"./platform/ttct/resources/assets/js/pages/article-create.js");
...
var _url_article_related_select =  listArticle.attr('data-reload');
var _per_page = 10; 
listArticle.select2({
  ajax: {
